...
cure<-py_to_r(pyclustering$cluster$cure)
...
```{r}
obj<-dice(colon_data,nk=10:20,reps=10, algorithms = c("hc","km","diana"), cons.funs = c("majority","CSPA","LCE"), progress = TRUE , evaluate = F ,prep.data = "full")
```
```{r}
obj$clusters
```
...
```{r}
#consensus clustering with another function
obj<-dice(colon_data,nk=10:20,reps=10, algorithms = c("km","diana","cure"), cons.funs = c("majority","CSPA","LCE"), progress = TRUE , evaluate = F ,prep.data = "full")
```
